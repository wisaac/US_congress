...
test2 <- train[which.na(train[[i]]),]
test <- train[ which.na(train$policy),]	
train <- train[ which(train$ideol == 1 | train$ideol == 0),]
plat_test <- test[1:28926,]
plat_train <- train[1:1485,]
...
t <- train_size+1
container <- create_container(DTM, platform$ideol, trainSize=plat_test$ideol, testSize=1486:28926, virgin=TRUE)
